...
def test_borrow_and_return():
    ...
    member_pk = manager.register_member("M002", "Charlie", "charlie@example.com", "555-9999")
    ...
    active_borrowing = [b for b in borrowings if b["status"] == "active"]
    assert len(active_borrowing) == 1
    borrowing_id = active_borrowing[0]["id"]
    
    success, msg = manager.return_book(borrowing_id)
    assert success is True
...
def test_unavailable_book_prevention():
    ...
    member_pk = manager.register_member("M003", "David", "david@example.com", "555-0000")
...
def test_duplicate_borrow_prevention():
    ...
    member_pk = manager.register_member("M004", "Alice", "alice@example.com", "555-1111")
