...
with tabs[1]:
    ...
    with sub_tabs[0]:
        ...
        search_query = col1.text_input(
            "Search books (Title, Author, ISBN)", key="book_search"
        )

        all_books = db.get_all_books()
        categories = ["All"] + sorted({b["category"] for b in all_books})
        selected_cat = col2.selectbox("Filter by Category", categories)

        filtered = manager.search_books(search_query)
        if selected_cat != "All":
            filtered = [
                b for b in filtered if b["category"].lower() == selected_cat.lower()
            ]
    ...
    with sub_tabs[2]:
        ...
        if all_books:
            book_options = {
                f"{b['title']} (ISBN: {b['isbn']})": b["id"] for b in all_books
            }
            ...
            with st.form("edit_book_form"):
                ...
                e_qty = st.number_input(
                    "Total Quantity", min_value=0, value=book_data["total_qty"], step=1
                )
...
with tabs[2]:
    ...
    sub_tabs = st.tabs(
        ["View & Search Members", "Register Member", "Edit / Delete Member"]
    )

    with sub_tabs[0]:
        search_query = st.text_input(
            "Search members (ID, Name, Email)", key="member_search"
        )
    ...
    with sub_tabs[2]:
        ...
        if all_members:
            member_options = {
                f"{m['name']} ({m['member_id']})": m["id"] for m in all_members
            }
            selected_m_label = st.selectbox(
                "Select Member", list(member_options.keys())
            )
...
with tabs[3]:
    ...
    with b_sub_tabs[0]:
        ...
        available_books = [b for b in db.get_all_books() if b["available_qty"] > 0]
        all_members = db.get_all_members()
        ...
        if not available_books:
            ...
            with st.form("borrow_form"):
                book_opts = {
                    f"{b['title']} (Available: {b['available_qty']})": b["id"]
                    for b in available_books
                }
                selected_book_label = st.selectbox(
                    "Select Book", list(book_opts.keys())
                )
    ...
    with b_sub_tabs[1]:
        ...
        active_borrowings = [
            b for b in db.get_all_borrowings() if b["status"] == "active"
        ]
        if active_borrowings:
            borrow_opts = {
                f"Book: {b['book_title']} | Member: {b['member_name']} (Due: {b['due_date']})": b[
                    "id"
                ]
                for b in active_borrowings
            }
            selected_borrow_label = st.selectbox(
                "Select Active Loan", list(borrow_opts.keys())
            )
...
with tabs[4]:
    ...
    if all_borrowings:
        filter_status = st.selectbox(
            "Filter Records by Status", ["All", "Active", "Returned", "Overdue"]
        )
