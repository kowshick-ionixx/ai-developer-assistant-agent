...
class Database:
    ...
    def update_book(self, book_id, title, author, isbn, category, quantity):
        with self.get_connection() as conn:
            ...
            if not row:
                return False
            old_qty, old_avail = row["total_qty"], row["available_qty"]
    ...
    def get_member_by_member_id(self, member_id_str):
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM members WHERE member_id = ?", (member_id_str,)
            )
            row = cursor.fetchone()
            return dict(row) if row else None
    ...
    def delete_member(self, member_id_pk):
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT COUNT(*) as count FROM borrowings WHERE member_id = ? AND status = 'active'",
                (member_id_pk,),
            )
            if cursor.fetchone()["count"] > 0:
                raise ValueError(
                    "Cannot delete a member with active borrowing records."
                )
    ...
    def return_book(self, borrowing_id, return_date):
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM borrowings WHERE id = ? AND status = 'active'",
                (borrowing_id,),
            )
