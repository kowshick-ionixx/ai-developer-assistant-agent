...
from employee import add_employee, get_employees, update_employee, delete_employee
...
if choice == "Dashboard":
    ...
    avg_salary = sum(e["salary"] for e in employees) / total_employees if total_employees > 0 else 0.0
    ...
    with tab1:
        ...
        if employees:
            ...
            emp_data = next((e for e in employees if e["employee_id"] == selected_emp_id), None)
            ...
            if emp_data:
                with st.form("edit_employee_form"):
                    ...
                    current_dept_idx = dept_names.index(emp_data["department"]) if emp_data["department"] in dept_names else 0
                    e_dept = st.selectbox("Department", dept_names, index=current_dept_idx) if dept_names else st.text_input("Department", value=emp_data["department"])
                    
                    e_desig = st.text_input("Designation", value=emp_data["designation"])
                    e_salary = st.number_input("Salary ($)", value=float(emp_data["salary"]), min_value=0.0, step=100.0)
                    e_joining = st.text_input("Joining Date (YYYY-MM-DD)", value=emp_data["joining_date"])
    ...
    with tab2:
        if not dept_names:
            ...
            with st.form("add_employee_form", clear_on_submit=True):
                ...
                new_joining = st.text_input("Joining Date (YYYY-MM-DD)", placeholder="2023-01-15")
    ...
    with tab1:
        ...
        if departments:
            for dept in departments:
                ...
                with col2:
                    new_dept_name = st.text_input(f"Rename {dept['name']}", value=dept['name'], key=f"rename_{dept['id']}")
                with col3:
