...
def add_employee(data, db_path="ems.db"):
    ...
    required = ["employee_id", "name", "email", "phone", "department", "designation", "salary", "joining_date"]
...
def update_employee(employee_id, data, db_path="ems.db"):
    required = ["name", "email", "phone", "department", "designation", "salary", "joining_date"]
    for field in required:
        if not data.get(field) or not str(data[field]).strip():
            return False, f"Field '{field}' cannot be empty."
...
def delete_employee(employee_id, db_path="ems.db"):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("DELETE FROM employees WHERE employee_id = ?", (str(employee_id).strip(),))
        conn.commit()
